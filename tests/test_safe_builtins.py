...
def test_safe_print():
    # Test successful case
    result = safe_print("Hello, world!")
    assert isinstance(result, Ok)

    # Test error case with invalid argument
    # This might be tricky to test since print is quite permissive
    # We can test with a custom object that raises an error in __str__
    class NotStringable:
        def __str__(self):
            raise TypeError("Cannot convert to string")

    result = safe_print(NotStringable())
    assert isinstance(result, Err)


def test_safe_hasattr():
    # Test successful case - attribute exists
    class TestClass:
        def __init__(self):
            self.test_attr = "exists"

    obj = TestClass()
    result = safe_hasattr(obj, "test_attr")
    assert isinstance(result, Ok)
    assert result.unwrap() is True

    # Test successful case - attribute doesn't exist
    result = safe_hasattr(obj, "non_existent_attr")
    assert isinstance(result, Ok)
    assert result.unwrap() is False

    # Test error case with invalid attribute name
    result = safe_hasattr(obj, 123)  # Not a string
    assert isinstance(result, Err)
